-- Test might not have been ran correctly
-- Does Message Rate translate to real significance, does other metrics also improve
-- Is data recorded correctly
...
-- Check other events such as like_message, view_inbox, and login to see if same trend applies, and it appears that trend does appear to those events as well

SELECT a.experiment,
       a.experiment_group,
       DATE_TRUNC('week', a.activated),
       COUNT(a.user_id) AS users,
       AVG(a.metric) AS average,
       SUM(a.metric) AS total
  FROM (
SELECT ex.experiment,
  ...
  JOIN tutorial.yammer_events e
   ...
   AND e.occurred_at < '2014-07-01'
   AND e.event_type = 'engagement'
   AND e.event_name = 'view_inbox'
   AND u.created_at > '2013-07-01'
 GROUP BY 1,2,3,4,5
       ) a
 GROUP BY 1,2,3
 
 -- Checking if there is any bias users in experiment, it looks like control group has alot of new users, and looks like it is skewing the data

 -- Recommendation is to randomly distribute users into control and test group and rerun the experiment